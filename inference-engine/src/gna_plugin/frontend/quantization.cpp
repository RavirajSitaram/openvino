...
float getRowMax(float* nums, uint32_t rows, uint32_t cols) {
    // calculate row max
    float* array = &nums[0];
...
void QuantizeAffine8_sse(float *ptr_float_weights, float *ptr_float_biases,
    ...
    for (uint32_t row = 0; row < num_rows; row++) {
        float scaled_row_max = getRowMax(ptr_float_weights + row * num_columns, num_rows, num_columns) * *ptr_weight_scale_factor;
